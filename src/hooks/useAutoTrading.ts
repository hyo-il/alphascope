...
/**
 * 자동매매 **상태만** 읽는다 (설정은 받지 않는다).
 *
 * 오른쪽 계좌 탭이 계좌 이름 옆 점 하나를 그리기 위해 쓴다.
 * ⚠️ 새 폴링을 만들지 않는다 — `useAutoTrading` 과 **같은 15초**이고, `active` 가 false 면
 * (패널이 접혀 있거나 다른 탭) 아무것도 부르지 않는다. 보이지 않는 값에 요청을 쓰지 않는
 * 기존 규칙 그대로다.
 */
export function useAutoTradeStatusOnly(accountId: number | null, active: boolean) {
  const [status, setStatus] = useState<AccountStrategyStatus | null>(null);
  const [strategy, setStrategy] = useState<AccountStrategy | null>(null);
  ...
  useEffect(() => {
    ...
    const tick = async (force = false) => {
      ...
      try {
        // 이 라우트는 설정과 상태를 함께 준다 — 점 하나에 필요한 enabled 도 여기 있다.
        const data = await json<{ strategy: AccountStrategy; status: AccountStrategyStatus }>(
          `/api/auto-trading/strategies/${accountId}`,
        );
        if (cancelled) return;
        ...
        // 점이 안 보이는 것뿐이다 — 계좌 요약을 깨뜨리지 않는다.
      }
    };

    void tick(true);
    const timer = setInterval(() => void tick(), STATUS_POLL_MS);
    // 다시 보이는 순간 즉시 한 번 — 멈춘 값이 15초 더 남아 있으면 꺼진 것처럼 읽힌다.
    const onVisible = () => {
